...
def test_limit_greater_than_1000_is_rejected(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="limit must be between 1 and 1000"):
        PilotConfig(limit=1001, output_dir=_output_dir(tmp_path))
...
def test_cumulative_400_limit_requires_eight_second_delay(tmp_path: Path) -> None:
    config = PilotConfig(limit=400, delay_seconds=8, output_dir=_output_dir(tmp_path))
    ...
    with pytest.raises(ValueError, match="delay_seconds must be >= 8 for cumulative limits above 100"):
        PilotConfig(limit=400, delay_seconds=5, output_dir=_output_dir(tmp_path))


def test_cumulative_700_limit_requires_eight_second_delay(tmp_path: Path) -> None:
    config = PilotConfig(limit=700, delay_seconds=8, output_dir=_output_dir(tmp_path))

    assert config.limit == 700
    assert config.max_limit == 1000
    assert config.concurrency == 1
    assert config.delay_seconds == 8

    with pytest.raises(ValueError, match="delay_seconds must be >= 8 for cumulative limits above 100"):
        PilotConfig(limit=700, delay_seconds=5, output_dir=_output_dir(tmp_path))


def test_cumulative_1000_limit_requires_eight_second_delay(tmp_path: Path) -> None:
    config = PilotConfig(limit=1000, delay_seconds=8, output_dir=_output_dir(tmp_path))
    ...
    with pytest.raises(ValueError, match="delay_seconds must be >= 8 for cumulative limits above 100"):
        PilotConfig(limit=1000, delay_seconds=5, output_dir=_output_dir(tmp_path))


def test_max_limit_greater_than_1000_is_rejected(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="max_limit must be <= 1000"):
        PilotConfig(limit=1000, max_limit=1001, delay_seconds=8, output_dir=_output_dir(tmp_path))
...
def test_cumulative_resume_to_400_counts_existing_and_new_articles(tmp_path: Path) -> None:
    output_dir = _output_dir(tmp_path)
    existing_urls = [f"https://spaces.ac.cn/archives/{20000 + index}" for index in range(200)]
    seed_urls = [f"https://spaces.ac.cn/archives/{21000 + index}" for index in range(220)]

    first_pilot = FullCorpusPilot(
        PilotConfig(limit=200, output_dir=output_dir, delay_seconds=8, seed_urls=tuple(existing_urls)),
        fetch_xml=lambda _url: _rss_fixture_for_urls([]),
        browser_fetcher=RecordingFetcher(),
        robots_allowed=lambda _urls: True,
        sleep=lambda _seconds: None,
    ...
    second_fetcher = RecordingFetcher()
    second_pilot = FullCorpusPilot(
        PilotConfig(limit=400, output_dir=output_dir, delay_seconds=8, seed_urls=tuple(seed_urls)),
        fetch_xml=lambda _url: (_ for _ in ()).throw(AssertionError("seed file should drive 400-article cumulative discovery")),
        browser_fetcher=second_fetcher,
        robots_allowed=lambda _urls: True,
        sleep=lambda _seconds: None,
    )
    ...
    assert summary.imported_count == 400
    assert summary.attempted_count == 200
    assert summary.skipped_count == 200
    assert summary.request_delay_seconds == 8
    assert summary.concurrency == 1
    assert second_fetcher.calls == seed_urls[:200]


def test_cumulative_400_idempotent_rerun_uses_local_runtime_only(tmp_path: Path) -> None:
    output_dir = _output_dir(tmp_path)
    seed_urls = [f"https://spaces.ac.cn/archives/{22000 + index}" for index in range(400)]
    ...
    second_fetcher = RecordingFetcher()
    second_pilot = FullCorpusPilot(
        PilotConfig(limit=400, output_dir=output_dir, delay_seconds=8),
        fetch_xml=lambda _url: (_ for _ in ()).throw(AssertionError("satisfied 400-article runtime state should not refetch source")),
        browser_fetcher=second_fetcher,
        robots_allowed=lambda _urls: True,
        sleep=lambda _seconds: None,
    )
    ...
    assert summary.imported_count == 400
    assert summary.attempted_count == 0
    assert summary.skipped_count == 400
    assert second_fetcher.calls == []


def test_cumulative_resume_to_700_counts_existing_and_new_articles(tmp_path: Path) -> None:
    output_dir = _output_dir(tmp_path)
    existing_urls = [f"https://spaces.ac.cn/archives/{25000 + index}" for index in range(400)]
    seed_urls = [f"https://spaces.ac.cn/archives/{26000 + index}" for index in range(320)]

    first_pilot = FullCorpusPilot(
        PilotConfig(limit=400, output_dir=output_dir, delay_seconds=8, seed_urls=tuple(existing_urls)),
        fetch_xml=lambda _url: _rss_fixture_for_urls([]),
        browser_fetcher=RecordingFetcher(),
        robots_allowed=lambda _urls: True,
        sleep=lambda _seconds: None,
    ...
    second_fetcher = RecordingFetcher()
    second_pilot = FullCorpusPilot(
        PilotConfig(limit=700, output_dir=output_dir, delay_seconds=8, seed_urls=tuple(seed_urls)),
        fetch_xml=lambda _url: (_ for _ in ()).throw(AssertionError("seed file should drive 700-article cumulative discovery")),
        browser_fetcher=second_fetcher,
        robots_allowed=lambda _urls: True,
        sleep=lambda _seconds: None,
    )
    ...
    assert summary.status == "PASS"
    assert summary.selected_count == 700
    assert summary.imported_count == 700
    ...
    assert second_fetcher.calls == seed_urls[:300]


def test_cumulative_700_idempotent_rerun_uses_local_runtime_only(tmp_path: Path) -> None:
    output_dir = _output_dir(tmp_path)
    seed_urls = [f"https://spaces.ac.cn/archives/{27000 + index}" for index in range(700)]
    ...
    second_fetcher = RecordingFetcher()
    second_pilot = FullCorpusPilot(
        PilotConfig(limit=700, output_dir=output_dir, delay_seconds=8),
        fetch_xml=lambda _url: (_ for _ in ()).throw(AssertionError("satisfied 700-article runtime state should not refetch source")),
        browser_fetcher=second_fetcher,
        robots_allowed=lambda _urls: True,
        sleep=lambda _seconds: None,
    )
    ...
    assert summary.imported_count == 700
    assert summary.attempted_count == 0
    assert summary.skipped_count == 700
    assert summary.max_consecutive_failures == 5
    assert summary.consecutive_failure_peak == 0
    assert second_fetcher.calls == []


def test_cumulative_resume_to_1000_counts_existing_and_new_articles(tmp_path: Path) -> None:
    output_dir = _output_dir(tmp_path)
    existing_urls = [f"https://spaces.ac.cn/archives/{30000 + index}" for index in range(700)]
    seed_urls = [f"https://spaces.ac.cn/archives/{31000 + index}" for index in range(320)]

    first_pilot = FullCorpusPilot(
        PilotConfig(limit=700, output_dir=output_dir, delay_seconds=8, seed_urls=tuple(existing_urls)),
        fetch_xml=lambda _url: _rss_fixture_for_urls([]),
        browser_fetcher=RecordingFetcher(),
        robots_allowed=lambda _urls: True,
        sleep=lambda _seconds: None,
    ...
    second_fetcher = RecordingFetcher()
    second_pilot = FullCorpusPilot(
        PilotConfig(limit=1000, output_dir=output_dir, delay_seconds=8, seed_urls=tuple(seed_urls)),
        fetch_xml=lambda _url: (_ for _ in ()).throw(AssertionError("seed file should drive 1000-article cumulative discovery")),
        browser_fetcher=second_fetcher,
        robots_allowed=lambda _urls: True,
        sleep=lambda _seconds: None,
    )
    ...
    assert summary.status == "PASS"
    assert summary.selected_count == 1000
    assert summary.imported_count == 1000
    assert summary.attempted_count == 300
    assert summary.skipped_count == 700
    assert summary.request_delay_seconds == 8
    assert summary.concurrency == 1
...
def test_cumulative_1000_idempotent_rerun_uses_local_runtime_only(tmp_path: Path) -> None:
    output_dir = _output_dir(tmp_path)
    seed_urls = [f"https://spaces.ac.cn/archives/{32000 + index}" for index in range(1000)]
    ...
    second_fetcher = RecordingFetcher()
    second_pilot = FullCorpusPilot(
        PilotConfig(limit=1000, output_dir=output_dir, delay_seconds=8),
        fetch_xml=lambda _url: (_ for _ in ()).throw(AssertionError("satisfied 1000-article runtime state should not refetch source")),
        browser_fetcher=second_fetcher,
        robots_allowed=lambda _urls: True,
        sleep=lambda _seconds: None,
    )
    ...
    assert summary.imported_count == 1000
    assert summary.attempted_count == 0
    assert summary.skipped_count == 1000
    assert summary.max_consecutive_failures == 5
    assert summary.consecutive_failure_peak == 0
    assert second_fetcher.calls == []
...
def test_cumulative_400_stops_at_bounded_candidate_window(tmp_path: Path) -> None:
    output_dir = _output_dir(tmp_path)
    existing_urls = [f"https://spaces.ac.cn/archives/{23000 + index}" for index in range(200)]
    failing_seed_urls = [f"https://spaces.ac.cn/archives/{24000 + index}" for index in range(400)]
    ...
    fetcher = RecordingFetcher({url: "TimeoutError: navigation timed out" for url in failing_seed_urls})
    second_pilot = FullCorpusPilot(
        PilotConfig(limit=400, output_dir=output_dir, delay_seconds=8, seed_urls=tuple(failing_seed_urls)),
        fetch_xml=lambda _url: _rss_fixture_for_urls([]),
        browser_fetcher=fetcher,
        robots_allowed=lambda _urls: True,
        sleep=lambda _seconds: None,
...
def test_cumulative_700_stops_at_bounded_candidate_window(tmp_path: Path) -> None:
    output_dir = _output_dir(tmp_path)
    existing_urls = [f"https://spaces.ac.cn/archives/{28000 + index}" for index in range(400)]
    failing_seed_urls = [f"https://spaces.ac.cn/archives/{29000 + index}" for index in range(700)]
    ...
    fetcher = RecordingFetcher({url: "TimeoutError: navigation timed out" for url in failing_seed_urls})
    second_pilot = FullCorpusPilot(
        PilotConfig(limit=700, output_dir=output_dir, delay_seconds=8, seed_urls=tuple(failing_seed_urls)),
        fetch_xml=lambda _url: _rss_fixture_for_urls([]),
        browser_fetcher=fetcher,
        robots_allowed=lambda _urls: True,
        sleep=lambda _seconds: None,
    ...
    assert summary.browser_transient_failures == 5
    assert summary.max_consecutive_failures == 5
    assert summary.consecutive_failure_peak == 5
    assert len(fetcher.calls) == 5


def test_cumulative_1000_stops_at_bounded_candidate_window(tmp_path: Path) -> None:
    output_dir = _output_dir(tmp_path)
    existing_urls = [f"https://spaces.ac.cn/archives/{33000 + index}" for index in range(700)]
    failing_seed_urls = [f"https://spaces.ac.cn/archives/{34000 + index}" for index in range(1000)]
    ...
    fetcher = RecordingFetcher({url: "TimeoutError: navigation timed out" for url in failing_seed_urls})
    second_pilot = FullCorpusPilot(
        PilotConfig(limit=1000, output_dir=output_dir, delay_seconds=8, seed_urls=tuple(failing_seed_urls)),
        fetch_xml=lambda _url: _rss_fixture_for_urls([]),
        browser_fetcher=fetcher,
        robots_allowed=lambda _urls: True,
        sleep=lambda _seconds: None,
