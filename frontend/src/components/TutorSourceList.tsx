    ...
    <section className={compact ? "mt-3 min-w-0" : "min-w-0 border-y border-slate-200 bg-white py-4"}>
      ...
      <div className="mt-3 grid gap-2">
        ...
        {bounded.visibleSources.map((source) => {
          ...
          return (
            <article key={`${source.source_type}-${source.source_id}`} className="rounded border border-slate-200 bg-white p-3 text-xs sm:text-sm">
              <p className="break-words font-medium">{safeTitle}</p>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {articleId ? (
                  <Link className="inline-block rounded border border-slate-200 px-2 py-1 text-xs text-slate-700 hover:border-slate-900" href={`/articles/${encodeURIComponent(articleId)}`}>
                    Open local article
                  </Link>
                ) : null}
                {externalUrl ? (
                  <a className="inline-block rounded border border-slate-200 px-2 py-1 text-xs text-slate-700 hover:border-slate-900" href={externalUrl} rel="noreferrer" target="_blank">
